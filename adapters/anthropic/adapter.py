...
#: Searches one escalation may run. Five was the first guess and it was too
#: mean by half: "suggest headphones and put a picture on the screen" spent
#: all five researching and then said its search quota had run out before it
#: could find a photograph — a correct and useless answer. Eight leaves room
...
SYSTEM = (
    ...
    "**Anything about a product, a price, a comparison, or what is true "
    "now, you look up — you do not answer it from memory.** Search, and "
    "when there are things to compare or a thing worth seeing, call "
    "`display` with a panel each and find a real photograph for them with "
    "`find_pictures`. A screen is standing right there; a list of names read "
    "aloud is the one answer it cannot help with.\n\n"
...
def declared_tools(granted):
    ...
    for t in granted:
        ...
        if t["name"] in ("display", "find_pictures"):
            ...
            # places is a schema that disagrees with itself.
            tools.append(t.get("schema") or {})
            continue
...
def run_once(client, run, inbox, dump=None):
    ...
    while True:
        ...
        # Web search's dynamic filtering runs code on their side, and a
        # custom tool called from inside that code is a *programmatic* call:
        # the container is paused waiting for the result, and the follow-up
        # request has to say which container. Without this the second request
        # is rejected outright —
        #
        #   container_id is required when there are pending tool uses
        #   generated by code execution with tools
        #
        # — which is a whole turn lost, after the search, for the one shape
        # this device most wants: look something up, then put it on screen.
        if container:
            # `container`, though the API's own error calls it container_id:
            # that is the wire field, and the SDK exposes it under the
            # shorter name. Passing container_id kills the adapter outright,
            # since an unexpected keyword is a TypeError and not a 400.
